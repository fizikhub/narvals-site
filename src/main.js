...
function initNarwhal3D(hero, scene) {
  const canvas = qs('[data-narwhal-canvas]', scene);
  ...
  const start = () => {
    ...
    hasStarted = true;
    void mountNarwhal3D(hero, scene, canvas).then((cleanup) => {
      dispose = cleanup;
    }).catch(() => {
      // The 2D mascot is intentionally kept as the no-WebGL/loading fallback.
      scene.classList.remove('is-3d-ready');
    });
  };

  // Keep the 2.5 MB model out of the critical hero request path until the
  // visitor is actually close to it. On a first load this resolves instantly.
...
async function mountNarwhal3D(hero, scene, canvas) {
  ...
  camera.position.set(0.08, 0.08, 3.15);
  camera.lookAt(0, 0, 0);
  webglScene.add(new AmbientLight(0xccecff, 2.35));
  const keyLight = new DirectionalLight(0xfffbdf, 3.4);
  keyLight.position.set(2.8, 3.8, 4.2);
  webglScene.add(keyLight);
  const rimLight = new DirectionalLight(0x67c7ea, 2.1);
  rimLight.position.set(-3.5, 1.2, -2.5);
  webglScene.add(rimLight);

  const modelGroup = new Group();
  modelGroup.rotation.set(0.08, -1.18, -0.14);
  modelGroup.scale.setScalar(1.26);
  modelGroup.position.set(0.12, -0.04, 0);
  webglScene.add(modelGroup);

  const loader = new GLTFLoader();
  loader.setMeshoptDecoder(MeshoptDecoder);
  const gltf = await loader.loadAsync('/assets/web/hero-narwhal-3d.glb');
  ...
  const resize = () => {
    ...
    width = nextWidth;
    height = nextHeight;
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, window.matchMedia('(pointer: coarse)').matches ? 1.3 : 1.7));
  ...
  const render = () => {
    ...
    modelGroup.rotation.y = -1.18 + (motionAllowed ? scrollProgress * 0.5 : 0);
    modelGroup.rotation.x = 0.08 + (motionAllowed ? scrollProgress * -0.1 : 0);
    modelGroup.rotation.z = -0.14 + (motionAllowed ? Math.sin(elapsed * 0.75) * 0.025 : 0);
    modelGroup.position.y = -0.04 + (motionAllowed ? Math.sin(elapsed * 0.9) * 0.035 : 0);
    renderer.render(webglScene, camera);
    if (motionAllowed) frame = requestAnimationFrame(render);
  };
  ...
  const scrollTween = gsap.to({}, {
    ...
    scrollTrigger: {
      ...
      onUpdate: (trigger) => {
        ...
        if (prefersReducedMotion && visible) renderer.render(webglScene, camera);
      }
    },
    onUpdate() {}
  });
  const motionChange = (event) => {
    if (!event.matches && visible) render();
    if (event.matches) {
      cancelAnimationFrame(frame);
      renderer.render(webglScene, camera);
    }
  ...
  scene.classList.add('is-3d-ready');
  cancelAnimationFrame(frame);
  render();
